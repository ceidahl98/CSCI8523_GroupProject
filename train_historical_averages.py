...
class SSTDataset(torch.utils.data.Dataset):  #dataset class
    ...
    def __getitem__(self, idx):
        ...
        times = torch.arange(t,t+t_window)
        # times = torch.tensor(t).repeat(4)
        # times = torch.tensor([x+i for x,i in enumerate(times)])
        if self.transform:
...
files = [r"data\1981_1985.nc", r"data\1986_1990.nc",
             ...
             r"data\2021_2024.nc"]
historical_averages_file = "historical_averages.npy"
# files = [r"D:\Datasets\data\1981_1985.nc", r"D:\Datasets\data\1986_1990.nc"]
...
print(val_dataset.data.min(axis=0),val_dataset.data.max(axis=0))
print(test_dataset.dataset)
print(test_dataset.data)
print(test_dataset.data.min(axis=0),test_dataset.data.max(axis=0))
...
ha_dataset = None
with open(historical_averages_file,"rb") as ha_fp:
    data = np.load(ha_fp)
    ha_dataset = HistoricalAverages(data)
    print(ha_dataset)
lats = [416, 416, 416, 416]
lons = [ 896,  896,  896,  896]
times = [4,5,6,7]
print(ha_dataset.get_historical_averages(times,lats,lons))
